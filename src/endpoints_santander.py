...
if bpm.service.text("STAGE") == "DEV":
    ...
    def login_apigee():
        ...
        url = "https://api.santander.com.ar/oauthv2/token"
        KEYS = get_pass_key()
        ...
        auth_headers = {
            ...
            'x-ibm-client-id': '9v7Z1boPQ5CkGzsAhOXMtpcWL0EBRXmo',
            'Accept': 'application/json',
    ...
    def save_suscription(auth_headers: dict, subscriptions: dict):
        ...
        url = "https://api.santander.com.ar/apif-api_mutual_funds/v2/subscriptions"
        return post(auth_headers,url, payload)

    def confirm_suscription(auth_headers:dict, transactionId):
        url = f"https://api.santander.com.ar/apif-api_mutual_funds/v2/subscriptions/{transactionId}?action=confirm"
        return put(auth_headers,url)

    def search_subscription(auth_headers:dict):
        url = "https://api.santander.com.ar/apif-api_mutual_funds/v2/subscriptions/search"
        return get(auth_headers, url)

    def get_subscription(auth_headers:dict, transactionId):
        url = f"https://api.santander.com.ar/apif-api_mutual_funds/v2/subscriptions/{transactionId}"
        return get(auth_headers, url)

    def save_redemption(auth_headers:dict, redemptions: dict):
        payload = json.dumps(redemptions)
        url = "https://api.santander.com.ar/apif-api_mutual_funds/v2/redemptions"
        return post(auth_headers, url, payload)

    def search_redemption(auth_headers:dict):
        # payload = json.dumps({})
        url = f"https://api.santander.com.ar/apif-api_mutual_funds/v2/redemptions/search"
        return get(auth_headers, url)

    def get_redemption(auth_headers:dict, transactionId):
        url = f"https://api.santander.com.ar/apif-api_mutual_funds/v2/redemptions/{transactionId}"
        return get(auth_headers, url)

    def confirm_redemption(auth_headers:dict, transactionId):
        url = f"https://api.santander.com.ar/apif-api_mutual_funds/v2/redemptions/{transactionId}?action=confirm"
        return put(auth_headers, url)

    def get_all_funds(auth_headers:dict):
        url = "https://api.santander.com.ar/apif-api_mutual_funds/v2/"
        return get(auth_headers, url)

    def get_fund_by_id(auth_headers:dict, fundId):
        url = f"https://api.santander.com.ar/apif-api_mutual_funds/v2/{fundId}"
        return get(auth_headers, url)

    def get_fund_by_id_details(auth_headers:dict, fundId):
        url = f"https://api.santander.com.ar/apif-api_mutual_funds/v2/{fundId}/details"
        return get(auth_headers, url)

    def get_fund_by_id_rules(auth_headers:dict, fundId):
        url = f"https://api.santander.com.ar/apif-api_mutual_funds/v2/{fundId}/rules"
        return get(auth_headers, url)
