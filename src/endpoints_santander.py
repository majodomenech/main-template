...
from key_n_account_data import get_pass_key, endpoint_calling_data



url_base, client_id = endpoint_calling_data()
def login_apigee():
    #payload is url encoded string
    payload = 'grant_type=client_credentials'
    url = f"{url_base}/oauthv2/token"
    KEYS = get_pass_key()
...
def save_suscription(auth_headers: dict, subscriptions: dict):
    ...
    url = f"{url_base}/apif-api_mutual_funds/v2/subscriptions"
    return post(auth_headers,url, payload)

def confirm_suscription(auth_headers:dict, transactionId):
    url = f"{url_base}/apif-api_mutual_funds/v2/subscriptions/{transactionId}?action=confirm"
    return put(auth_headers,url)

def search_subscription(auth_headers:dict):
    url = f"{url_base}/apif-api_mutual_funds/v2/subscriptions/search"
    return get(auth_headers, url)

def get_subscription(auth_headers:dict, transactionId):
    url = f"{url_base}/apif-api_mutual_funds/v2/subscriptions/{transactionId}"
    return get(auth_headers, url)

def save_redemption(auth_headers:dict, redemptions: dict):
    payload = json.dumps(redemptions)
    url = f"{url_base}/apif-api_mutual_funds/v2/redemptions"
    return post(auth_headers, url, payload)

def search_redemption(auth_headers:dict):
    # payload = json.dumps({})
    url = f"{url_base}/apif-api_mutual_funds/v2/redemptions/search"
    return get(auth_headers, url)

def get_redemption(auth_headers:dict, transactionId):
    url = f"{url_base}/apif-api_mutual_funds/v2/redemptions/{transactionId}"
    return get(auth_headers, url)

def confirm_redemption(auth_headers:dict, transactionId):
    url = f"{url_base}/apif-api_mutual_funds/v2/redemptions/{transactionId}?action=confirm"
    return put(auth_headers, url)

def get_all_funds(auth_headers:dict):
    url = f"{url_base}/apif-api_mutual_funds/v2/"
    return get(auth_headers, url)

def get_fund_by_id(auth_headers:dict, fundId):
    url = f"{url_base}/apif-api_mutual_funds/v2/{fundId}"
    return get(auth_headers, url)

def get_fund_by_id_details(auth_headers:dict, fundId):
    url = f"{url_base}/apif-api_mutual_funds/v2/{fundId}/details"
    return get(auth_headers, url)

def get_fund_by_id_rules(auth_headers:dict, fundId):
    url = f"{url_base}/apif-api_mutual_funds/v2/{fundId}/rules"
    return get(auth_headers, url)
